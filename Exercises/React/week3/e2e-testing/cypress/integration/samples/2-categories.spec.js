...
describe("Categories", () => {
  ...
  it("Selecting a new category should deselect the old one", () => {
    ...
    cy.get('[data-testid="categories-list"]').within(() => {
      ...
      cy.get('[data-selected="false"]').should("have.length", 4);
      // 2. Click a category
      // Select the category jewelery
      cy.get('[data-elementid="jewelery"]').click();
      // 3. Check that that category is selected
      cy.get('[data-selected="true"][data-elementid="jewelery"]').should("have.length", 1);
      // 4. Click a different category
      cy.get('[data-elementid="electronics"]').click();
      // 5. Check that only the new category is selected
      cy.get('[data-selected="false"][data-elementid="jewelery"]').should("have.length", 1);
      cy.get('[data-selected="true"][data-elementid="electronics"]').should("have.length", 1);
    });
  });
});
